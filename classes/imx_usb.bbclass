...
IMX_USB_UBOOT ?= "${UBOOT_BINARY}-${UBOOT_CONFIG}"
IMX_USB_SPL ?= "${SPL_BINARY}-${UBOOT_CONFIG}"
IMX_USB_VID ?= "${IMX_USB_RAW_VID}"
IMX_USB_PID ?= "${IMX_USB_RAW_PID}"
...
${IMX_USB_RAW_VID}:${IMX_USB_RAW_PID}, mx6_usb_work.conf
EOF
...
${IMX_USB_DTB}:load ${IMX_USB_DTB_LOADADDR}
${IMX_USB_INITRD}:load ${IMX_USB_INITRD_LOADADDR}
${IMX_USB_UBOOT}:clear_dcd,load,jump header
EOF
	fi
}
